	...
	refresh: function (frm) {
		...
		frm._showOrgPicker = function (leads) {
			...
			const rows = leads
				.map(
					(l) => `
				<div class="org-pick-row"
					data-org="${frappe.utils.escape_html(l.company_name)}"
					data-first="${frappe.utils.escape_html(l.first_name || "")}"
			...
			d.$body
				...
				.on("click", function () {
					frm.set_value("company_name", $(this).data("org"));
					if (!frm.doc.first_name && $(this).data("first")) {
						frm.set_value("first_name", $(this).data("first"));
					}
					d.hide();
				});
			d.show();
				...
				callback: function (r) {
					...
					if (unique.length === 1) {
						if (!frm.doc.company_name) {
							frm.set_value("company_name", unique[0].company_name);
						}
					} else {
		...
		frm._org_search_timer = setTimeout(() => {
			frappe.call({
				...
				callback: function (r) {
					...
					// Auto-fill first_name if only one unique result
					const unique = [...new Map(leads.map((l) => [l.company_name, l])).values()];
					if (unique.length === 1) {
						if (!frm.doc.first_name && unique[0].first_name) {
							frm.set_value("first_name", unique[0].first_name);
						}
					} else {
						frm._showOrgPicker && frm._showOrgPicker(unique);
					}
