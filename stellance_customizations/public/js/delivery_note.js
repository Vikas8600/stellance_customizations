frappe.ui.form.on('Delivery Note', {
	refresh: function (frm) {
        ...
        frm.fields_dict["items"].grid.add_custom_button(__('Select Packs'), 
			function() {
                                ...
                                frappe.call({
                                    ...
                                    callback: function (res) {
                                        if (res.message) {
                                            ...
                                            dialog.fields_dict.pack_sizes_table.df.cannot_delete_rows = !allow_delete;
                                            dialog.fields_dict.pack_sizes_table.grid.refresh();

                                            // if (!allow_delete) {
                                            //     // frappe.msgprint(__('You cannot delete rows in the pack sizes table '));
                                            // }
                                            ...
                                            if (item.custom_pack_size || (item.custom_pack_size && item.custom_product_bundle)) {
												...
												// // Insert one blank row into the Pack Sizes table.
												// dialog.fields_dict.pack_sizes_table.df.data = [{}];
												
                                                let so_qty = dialog.fields_dict.qty.get_value() || 0; 
                                                dialog.fields_dict.pack_sizes_table.df.data = [{
                                                    accepted_qty: so_qty 
                                                }];
                                                dialog.fields_dict.pack_sizes_table.grid.refresh();
                            ...
                            frappe.call({
                                ...
                                callback: function (r) {
                                    if (r.message) {
                                        ...
                                        table.data.forEach(row => {
                                            const match = stock_data.find(item => item.packs === row.item_name);
                                            if (match) {
                                                row.batch_id = match.batch_no;
                                            }
                                        });
                                        table.refresh();
                                    }
